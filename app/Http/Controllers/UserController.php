...
class UserController extends Controller
{
    ...
    function userSignup(Request $request)
    {
        ...
        if ($user) {
            ...
            if (Session::has('quiz-url')) {
                ...
                return redirect('/')->with('message-success', "Your account has been created. Please verify your email to activate your profile.");
            }
        }
    }
    ...
    function startQuiz($id, $name)
    {
        ...
        if (!Session::has('user')) {
            return redirect('user-login')->with('message-error', 'Please login to attempt the quiz.');
        }

        // 2. Check if user email is verified
        if (Session::get('user')->active != 2) {
            return redirect()->back()->with('message-error', 'Please verify your email to attempt the quiz.');
        }

        // 3. Check if quiz has MCQs
        $quizCount = Mcq::where('quiz_id', $id)->count();
        if ($quizCount === 0) {
            return redirect()->back()->with('message-error', 'No MCQs available for this quiz.');
        }
